...
void addUserToFile(const std::string& username) {
    // Add the username to the "usernames.user" file.
    std::ofstream usernameFile("usernames.user", std::ios::app);
    ...
    std::ofstream userFile(username + ".user");
    if (!userFile) {
...
std::pair<int, int> getUserData(const std::string& username) {
    std::ifstream userFile(username + ".user");
...
void updateUserData(const std::string& username, int newBestWPM, double newAccuracy) {
    std::ifstream usernameFile("usernames.user");
    if (!usernameFile) {
    ...
    std::string filename = username + ".user";
...
int main() {
    ...
    // Check if the username already exists in "usernames.user".
    std::ifstream usernameFile("usernames.user");
    if (!usernameFile) {
        std::cerr << "Error: Unable to open the username file for reading." << std::endl;
        return 1;
